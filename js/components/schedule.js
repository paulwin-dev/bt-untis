...
async function loadWeek(session, weekOffset, direction = 0) {
	...
	const headers = document.getElementById("schedule-day-headers"); //load days
	headers.textContent = "";
	...
	const timeslots = definition.slots  // full fixed slot list, always all periods
	const skip = new Set()
	...
	for (let slotIndex = 0; slotIndex < timeslots.length; slotIndex++) {
		...
		for (let day = 0; day < 5; day++) {
			...
			if (period) {
				const customData = await storage.getCustomPeriodData(dateKey, period.startTime)
				if (customData && customData.homework.length > 0) {
					cell.classList.add("homework")
				}
				...
				`
				cell.addEventListener("click", () => openDetailsPanel(period, dateKey, customData))
			} else {
...
grid.addEventListener('touchend', e => {
    ...
    loadWeek(session, weekOffset, diff)
})

window.addEventListener('popstate', e => {
    // add more panels/modals here as you add them
})
